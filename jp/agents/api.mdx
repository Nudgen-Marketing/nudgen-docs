...
draft の作成と lifecycle の変更には PAT を使用します（`PATCH /api/v1/campaigns/:id/lifecycle`）。`POST /api/v1/campaigns/:id/triggers` は `ndg_team_*` Team trigger API key 専用です。idempotency には `eventId` を使い、execution は `GET /api/v1/campaigns/:id/executions` で確認します。[Lifetime ガイド](/jp/campaigns/lifetime)を参照してください。
