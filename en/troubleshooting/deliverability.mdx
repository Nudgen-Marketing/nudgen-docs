...
- If **Campaign sending is paused** appears in **Settings** → **Custom domain**, read the reason, repair the sender's DNS or paid-plan access, and select **Resume sending**. Queued emails are preserved. See [Custom domain](/en/settings/sending-domains).

**Test email didn’t arrive**

- Check spam/junk. Add the sender to your contacts or safe senders.
- If you use a custom sender, confirm its domain is **Ready** in **Settings** → **Custom domain**. A missing DKIM or Return-Path record can block the test send. See [Custom domain](/en/settings/sending-domains).
