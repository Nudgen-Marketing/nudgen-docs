...
- Contacts added through [CSV or manual entry](/en/contacts/upload) or an [available integration](/en/integrations/connect-and-sync)
...
If you connected a provider, finish its audience import first. A connected provider does not automatically add every customer to your Nudgen contacts.
...
Check the sender before confirming. If your workspace uses a [custom sending domain](/en/settings/sending-domains), its default sender must still be verified and eligible to send. A sender hold pauses campaign delivery until you fix the cause and explicitly resume sending.
