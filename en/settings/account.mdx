...
| **API Keys** | Create and revoke Personal Access Tokens for REST and CLI workflows; manage connected MCP apps | [Developer API](/en/agents/api), [MCP server](/en/agents/mcp) |
| **Integration** | Connect a contact source and manage syncing | [Integrations](/en/integrations/overview) |
| **Custom domain** | Verify a sending domain and choose your sender | [Custom sending domain](/en/settings/sending-domains) |
...
Use **Settings** → **API Keys** to create Personal Access Tokens. PATs authenticate the Developer API and CLI. Copy a token once when it is created, store it securely, and revoke it when a script or agent no longer needs access. MCP clients use OAuth; manage them under **Connected MCP apps** on the same page.
...
<CardGroup cols={2}>
  ...
  <Card title="MCP server" icon="bot" href="/en/agents/mcp">
    Tool-based agent access with OAuth.
  </Card>
