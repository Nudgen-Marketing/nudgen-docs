---
title: "Developer API"
description: "REST API and MCP access with Personal Access Token authentication. Manage contacts, campaigns, brand settings, and AI drafts programmatically."
---

## Overview

The Nudgen Developer API gives scripts, internal tools, and AI agents a predictable way to work with your Nudgen workspace. Use REST endpoints for normal HTTP integrations, or use MCP when an agent can call Nudgen tools directly.

<CardGroup cols={3}>
  <Card title="PAT auth" icon="shield-check">
    Authenticate REST and MCP requests with the same Personal Access Token.
  </Card>
...
Use MCP when your agent supports Model Context Protocol tools.

```text
POST https://nudgen.net/api/mcp
GET  https://nudgen.net/.well-known/mcp/server-card.json
```

Authenticate MCP requests with the same bearer PAT:
