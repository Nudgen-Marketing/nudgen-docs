...
使用 PAT 创建 draft 和修改 lifecycle（`PATCH /api/v1/campaigns/:id/lifecycle`）。只有 `ndg_team_*` Team trigger API key 可以调用 `POST /api/v1/campaigns/:id/triggers`。使用 `eventId` 实现幂等，并通过 `GET /api/v1/campaigns/:id/executions` 查询 execution。参阅 [Lifetime 指南](/cn/campaigns/lifetime)。
