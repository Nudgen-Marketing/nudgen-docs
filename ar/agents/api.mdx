...
استخدم PAT لإنشاء draft وتغيير lifecycle عبر `PATCH /api/v1/campaigns/:id/lifecycle`. لا يمكن استدعاء `POST /api/v1/campaigns/:id/triggers` إلا باستخدام Team trigger API key يبدأ بـ `ndg_team_*`. استخدم `eventId` لضمان idempotency واستعلم عن execution عبر `GET /api/v1/campaigns/:id/executions`. راجع [دليل Lifetime](/ar/campaigns/lifetime).
